...
from MMA_gym_env import MMAGymEnv

# === Build training and evaluation environments ===
def make_train_env(n_envs=4, seed=42, adversary_model_path=None):
    def _make_env():
        return Monitor(MMAGymEnv(adversary_model=adversary_model_path))
    envs = DummyVecEnv([_make_env for _ in range(n_envs)])
    envs.seed(seed)
    return VecNormalize(envs, norm_obs=True, norm_reward=True)

def make_eval_env(n_envs=1, seed=42, adversary_model_path=None):
    def _make_env():
        return Monitor(MMAGymEnv(adversary_model=adversary_model_path))
    envs = DummyVecEnv([_make_env for _ in range(n_envs)])
    envs.seed(seed)
...
class MarketLoggingCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        for i, info in enumerate(infos):
            if i != 0:
                continue # Restrict logging to env_0 to reduce redundancy
            if self.timestep % self.print_freq != 0:
                self.timestep += 1
                continue  # Only log once every print_freq steps


            state = info.get("market_state", {})
            total_arrivals = info.get("total_arrivals", 0)

            # Write logs to file (env_id is fixed to 0)
            ...
            self.market_log.write(f"{self.timestep}_env0," + ",".join(str(state.get(k, 0.0)) for k in [
                ...
                "inventory_A", "cash_A", "pnl_A",
                "bid_price_A", "ask_price_A",
                "bid_fill_A", "ask_fill_A"
            ]) + f",{total_arrivals}\n")
            ...
            line = (
                ...
                f"inventory={state.get('inventory_A', 0.0)} | cash={state.get('cash_A', 0.0):.2f} | "
                f"pnl={state.get('pnl_A', 0.0):.2f} | bid={state.get('bid_price_A', 0.0):.2f} | "
                f"ask={state.get('ask_price_A', 0.0):.2f} | bid_fill={state.get('bid_fill_A', 0.0)} | "
                f"ask_fill={state.get('ask_fill_A', 0.0)} | total_arrivals={total_arrivals}"
            )
            print(line)
...
def run_training(
    ...
    base_dir = os.path.join("Train_MMA", project_name)
    ...
    train_env = make_train_env(n_envs=n_envs, seed=seed, adversary_model_path=adversary_model_path)
    eval_env = make_eval_env(n_envs=1, seed=seed, adversary_model_path=adversary_model_path)
