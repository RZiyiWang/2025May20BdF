...
from B1_gym_env import B1GymEnv


def make_train_env(n_envs=4, seed=42, adversary_model_path=None, agent_A_path=None):
    def _make_env():
        return Monitor(B1GymEnv(adversary_model=adversary_model_path, agent_A=agent_A_path))
    ...
    def _on_step(self) -> bool:
        ...
        for i, info in enumerate(infos):
            ...
            self.market_log.write(f"{self.timestep}_env0," + ",".join(str(state.get(k, 0.0)) for k in [
                ...
                "inventory_B1", "cash_B1", "pnl_B1",
                "bid_price_B1", "ask_price_B1",
                "bid_fill_B1", "ask_fill_B1"
            ]) + f",{total_arrivals}\n")
            ...
            line = (
                ...
                f"inventory={state.get('inventory_B1', 0.0)} | cash={state.get('cash_B1', 0.0):.2f} | "
                f"pnl={state.get('pnl_B1', 0.0):.2f} | bid={state.get('bid_price_B1', 0.0):.2f} | "
                f"ask={state.get('ask_price_B1', 0.0):.2f} | bid_fill={state.get('bid_fill_B1', 0.0)} | "
                f"ask_fill={state.get('ask_fill_B1', 0.0)} | total_arrivals={total_arrivals}"
            )
            print(line)
...
def run_training(
    ...
    base_dir = os.path.join("Train_MMB1", project_name)
...
if __name__ == "__main__":
    run_training(
        ...
        train_steps=200,
        train_total=3400,
        load_path="/B1_new3200_total3200.zip",
